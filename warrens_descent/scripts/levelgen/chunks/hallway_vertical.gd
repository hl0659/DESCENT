...
extends ChunkBase
## A ramped hallway that rises 5 m over its length.
## 10 m wide, 35 m long total: 5 m flat bottom, 25 m ramp, 5 m flat top.
## Very high ceiling (13 m) so the ramp is never blocked. Floor at y=0.
...
func build() -> void:
	...
	#   Bottom flat: z = -17.5 to z = -12.5
	#   Ramp:        z = -12.5 to z = +12.5
	#   Top flat:    z = +12.5 to z = +17.5

	var bottom_z_start := -half_len
	var bottom_z_end := bottom_z_start + FLAT_BOTTOM
	var ramp_z_start := bottom_z_end
	var ramp_z_end := ramp_z_start + RAMP_LENGTH
	var top_z_start := ramp_z_end
	...
	MeshBuilder.add_doorway(self, Vector3(0, bottom_door_y, bottom_z_start), Vector3.BACK, "entry")
	MeshBuilder.add_doorway(self, Vector3(0, top_door_y, top_z_end), Vector3.FORWARD, "exit")
